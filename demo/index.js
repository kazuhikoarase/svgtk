...
window.addEventListener('load', function() {
  ...
  !function() {
    ...
    $(cb.$el).on('mousedown', function(event) {
      event.preventDefault();
      var dragPoint = { x : event.pageX, y : event.pageY };
      var dragAngles = { p : cb.model.angles.p, t : cb.model.angles.t };
      var mousemoveHandler = function(event) {
        var dx = event.pageX - dragPoint.x;
        var dy = event.pageY - dragPoint.y;
        cb.model.angles.p = dragAngles.p + dx * 0.5;
        cb.model.angles.t = dragAngles.t - dy * 0.5;
        cb.update();
      };
      var mouseupHandler = function(event) {
        $(document).off('mousemove', mousemoveHandler);
